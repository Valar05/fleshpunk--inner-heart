#!/usr/bin/env python3
"""Serve lightweight linked room/event docs for Fleshpunk content review."""
        ...
        if followups:
            ...
            for followup in followups:
                ...
                if isinstance(linked_event, dict) and linked_event:
                    lines.append(f"- [{followup_id}](/events/{event_slug(followup_id)}): {linked_event.get('line_1', '')}\n")
            lines.append("\n")
...
def event_markdown(store: ContentStore, event_id: str) -> str:
    ...
    if buttons:
        for button in buttons:
            if not isinstance(button, dict):
                continue
            lines.append(f"- **{button.get('label', '')}** -> `{button.get('action', '')}`\n")
    ...
    if followups:
        ...
        for followup in followups:
            ...
            action = str(followup.get("action", "default"))
            delay = followup.get("delay_rooms", "")
            trigger = followup.get("trigger_key", "")
            if followup_id:
                lines.append(f"- `{action}` -> [{followup_id}](/events/{event_slug(followup_id)})")
                details = []
                if delay != "":
    ...
    hidden_keys = {"id", "type", "speaker", "line_1", "line_2", "buttons", "story_followups"}
...
def main() -> int:
    ...
    print(f"Serving room docs at http://{args.host}:{args.port}/rooms")
    try:
